...
const M={cream:mat('cream','#e7e5e0',.08,.56),sage:mat('sage','#232b3c',.12,.62),dark:mat('dark','#171f2e',.2,.58),rubber:mat('rubber','#10161f',.02,.78),silver:mat('silver','#b7bac2',.88,.34),chrome:mat('chrome','#d8dae0',.96,.21),copper:mat('copper','#b4aa9c',.85,.33),brass:mat('brass','#c9c0b2',.78,.39),board:mat('board','#232b3c',.05,.78),track:mat('track','#6e7687',.38,.55),white:mat('white','#f5f4f1',0,.62),red:mat('red','#8a90a0',.06,.6),blue:mat('blue','#3c4453',.2,.45),wood:mat('wood','#1a2130',.0,.56),slot:mat('slot','#10161f',.1,.8)};
M.meter=mat('meter','#d3d1cf',.03,.7);M.meter.emissive=new T.Color('#2b3344');M.meter.emissiveIntensity=.25;
M.screen=mat('screen','#10161f',.3,.25);M.led=new T.MeshBasicMaterial({color:'#e7e5e0'});
...
function update(){if(opening!==lastShadowOpening){renderer.shadowMap.needsUpdate=true;lastShadowOpening=opening;}model.set(opening);const target=model.target;const a=.66+yaw,e=.41+pitch,r=11;camera.position.set(target.x+Math.sin(a)*Math.cos(e)*r,target.y+Math.sin(e)*r,target.z+Math.cos(a)*Math.cos(e)*r);camera.lookAt(target);camera.updateMatrixWorld();const b=model.bounds;let minX=Infinity,maxX=-Infinity,minY=Infinity,maxY=-Infinity;for(const x of [b.min.x,b.max.x])for(const y of [b.min.y,b.max.y])for(const z of [b.min.z,b.max.z]){temp.set(x,y,z).applyMatrix4(camera.matrixWorldInverse);minX=Math.min(minX,temp.x);maxX=Math.max(maxX,temp.x);minY=Math.min(minY,temp.y);maxY=Math.max(maxY,temp.y);}const aspect=stage.clientWidth/stage.clientHeight;const w=Math.max(maxX-minX,(maxY-minY)*aspect)*1.08;camera.left=-w/2;camera.right=w/2;camera.top=w/aspect/2;camera.bottom=-w/aspect/2;camera.updateProjectionMatrix();}
